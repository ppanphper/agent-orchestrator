...
Live merged sessions remain visible in the **Ready to merge / Merged** lane. Terminated runtimes, including terminated sessions whose PR is merged, appear in the collapsed **Archive** area and can be restored when you need to inspect or continue them. Expand Archive to switch between the default multi-column card layout and compact rows; AO remembers your choice.
