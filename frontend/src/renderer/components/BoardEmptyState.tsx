		...
		<div className="flex h-full min-h-0 items-center justify-center overflow-y-auto">
			<div className="flex w-full max-w-board-empty flex-col items-center pb-empty-offset-y text-center">
				...
				<h2 className="mt-5 text-heading-sm font-semibold tracking-tight-lg text-foreground">
					{t("Welcome to Agent Orchestrator")}
				</h2>
				<p className="mt-1.5 max-w-[320px] text-[12.5px] leading-[1.65] text-muted-foreground">
					{t("Add a repository and describe the work. AO runs agents on isolated branches, from start to merge.")}
				</p>

				<CreateProjectFlow
					idleLabel={t("Add your first project")}
				...
				>
					{({ choosePath, disabled, error, label }) => (
						<>
							<TopbarButton
								aria-label={t("Add your first project")}
								className="mt-7"
								disabled={disabled}
								onClick={choosePath}
								variant="primary"
				...
				<p className="mt-3 text-caption text-passive">
					{t("Adding a project starts its orchestrator - the agent you talk to.")}
				</p>
			</div>
		</div>
	);
		...
		<div className="flex h-full min-h-0 items-center justify-center overflow-y-auto">
			<div className="flex w-full max-w-preview-content flex-col items-center pb-empty-offset-y text-center">
				...
				<p className="mt-2 text-md-sm leading-relaxed text-muted-foreground">
					{t("Describe a task and the orchestrator plans it, spawns worker sessions, and tracks them here from work to merge.")}
				</p>
				<div className="mt-5 flex items-center gap-2">
					<TopbarButton
						aria-label={t(hasOrchestrator ? "Orchestrator" : "Spawn Orchestrator")}
					...
					<TopbarButton aria-label={t("New task")} disabled={isProjectRestarting} onClick={onNewTask} variant="accent">
						<Plus className="size-icon-md" aria-hidden="true" />
						{t("New task")}
					</TopbarButton>
				</div>
