...
export function ReportProblemDialog({ open, onOpenChange }: ReportProblemDialogProps) {
	...
	const copyDraft = async () => {
		...
		try {
			...
			setCopyError(err instanceof Error ? err.message : t("Could not copy report draft"));
			setCopiedOutput(null);
		}
	};
		...
		<Dialog open={open} onOpenChange={onOpenChange}>
			...
			>
				...
				<div className="flex shrink-0 flex-col gap-1 border-b border-(--color-border-settings-dialog-header) px-6 pt-5 pb-4">
					<DialogTitle className="settings-dialog-title">{t("Report a problem")}</DialogTitle>
					<DialogDescription className="text-control leading-4 text-settings-muted">
						{t("Found an issue? Tell us what happened.")}
					</DialogDescription>
				</div>
				...
				<div className="flex min-h-0 flex-col gap-4 overflow-y-auto px-6 pt-4 pb-5">
					...
					<div className="flex flex-col gap-1.5">
						...
						<textarea
							...
							placeholder={t("Share what happened, what you expected, and how to reproduce it.")}
						/>
					</div>
					...
					{copiedLabel && !copyError && (
						<p className="text-caption leading-4 text-success">
							{t("{destination} draft copied.", { destination: t(copiedLabel) })}
						</p>
					)}
				</div>
