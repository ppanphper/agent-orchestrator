	...
	return (
		<>
			<SettingsSection title={t("Updates")}>
				...
				<SettingsRow icon={History} label={t("Automatic Updates")}>
					<SettingsOptionMenu
				...
				<SettingsRow icon={HardDriveDownload} label={t("Updates channel")}>
					<SettingsOptionMenu
			...
			<ConfirmDialog
				...
				title={t("Switch feature build?")}
				description={
					pendingPin
						? t("Switch to PR #{pr}: {title}? The app will download the feature build and restart.", {
								pr: pendingPin.pr,
...
function FeatureBuildsSelect({
	...
	if (!buildsQuery.isLoading && builds.length === 0) {
		return (
			<div className="px-1 text-xs text-settings-muted">
				<span className="sr-only">{t("Feature build")}</span>
				{t("No live feature releases.")}
			</div>
		);
	}
	...
	return (
		<SettingsRow icon={GitPullRequest} label={t("Feature build")}>
			<SettingsOptionMenu
				aria-label={t("Feature build")}
				value={currentPr === null ? "__none__" : currentPr.toString()}
				placeholder={t("Select a feature build...")}
				options={options}
				...
				<div className="settings-row-bar h-auto min-h-0 flex-wrap justify-start gap-3 py-3">
					{status.state === "available" && (
						<Button type="button" variant="primary" onClick={() => void aoBridge.updates.download()}>
							{t("Update to {version}", { version: status.version ? `v${status.version}` : t("latest") })}
						</Button>
					)}
					{status.state === "downloaded" && (
...
function UpdateStatusLine({ status }: { status: UpdateStatus }) {
	...
	switch (status.state) {
		case "checking":
			return <span className="text-xs text-settings-muted">{t("Checking for updates...")}</span>;
		case "available":
			return (
				<span className="text-xs text-settings-muted">
					{t("Update available{version}.", { version: status.version ? ` (v${status.version})` : "" })}
				</span>
			);
		case "downloading":
			return (
				<span className="text-xs text-settings-muted">
					{t("Downloading... {percent}%", { percent: status.percent ?? 0 })}
				</span>
			);
		case "downloaded":
			return <span className="text-xs text-success">{t("Downloaded. Restart to finish updating.")}</span>;
		case "not-available":
			return <span className="text-xs text-settings-muted">{t("You're on the latest version.")}</span>;
		case "unsupported":
			return (
				<span className="text-xs text-settings-muted">{status.message ?? t("Updates need the installed app.")}</span>
			);
		case "error":
			return <span className="text-xs text-error">{status.message ?? t("Update failed.")}</span>;
		default:
			return null;
	}
}
