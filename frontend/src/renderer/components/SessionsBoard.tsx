	...
	return (
		...
		>
			<button
				...
				aria-label={`${t("Idle sessions")} (${sessions.length})`}
				className={cn(
			...
			>
				...
				<span className="font-mono text-2xs font-semibold uppercase tracking-wide-md">{t("Idle")}</span>
		...
		>
			...
			{restoreAction && (
				<button
					...
					type="button"
				>
					{t(isRestoring ? "Restoring" : "Restore")}
				</button>
			)}
		</div>
	);
