...
export function DegradedProjectState({
  ...
  const resolvedHeading = heading ?? t("projects.degradedHeading");
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-[var(--color-text-tertiary)]">
              {t("projects.degradedProject")}
            </p>
            <h1 className="mt-2 text-2xl font-semibold">{resolvedHeading}</h1>
            <p className="mt-2 text-sm text-[var(--color-text-secondary)]">
              {t("projects.degradedDescription", { project: projectId })}
            </p>
          </div>
        </div>
