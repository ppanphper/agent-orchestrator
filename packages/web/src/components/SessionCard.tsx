...
function getPRStatusLabel(
  ...
  if (p.state === "merged") return t("session.merged");
  if (p.state === "closed") return t("session.closed");
  if (p.ciStatus === "failing") return t("session.ciFailing");
  if (p.reviewDecision === "changes_requested") return t("session.changesRequested");
  if (p.isDraft) return t("session.draft");
  if (p.reviewDecision === "approved") return t("session.approved");
  if (p.ciStatus === "passing") return t("session.needsReview");
  if (p.ciStatus === "pending") return t("session.ciPending");
  return "";
}
...
function SessionCardView({ session, onKill, onMerge, onRestore }: SessionCardProps) {
  ...
  const footerDetail = getFooterDetail(
    session,
      ...
      <div className="session-card__body flex min-h-0 flex-1 flex-col">
        ...
        {prs.length > 1 && (
          <div className="px-[10px] pb-[5px] flex flex-col gap-[2px]">
            {prs.map((p, i) => {
              const statusLabel = getPRStatusLabel(p, t);
              ...
              return (
                ...
                >
                  ...
                  {statusLabel && statusLabel !== t("session.needsReview") && (
          ...
          <div className="session-card__footer-actions">
            {isReadyToMerge && effectivePR ? (
              ...
              >
                ...
                {t("session.mergePr", { number: effectivePR.number })}
              </button>
            ) : null}
            {!isTerminal ? (
              <button
...
function getFooterDetail(
  ...
  if (!pr) {
    if (session.lifecycle?.sessionState === "detecting") {
      return { text: t("session.detecting"), tone: undefined };
    }
    return { text: t("session.noPrYet"), tone: undefined };
  }
  if (rateLimited) return { text: t("session.prRateLimited"), tone: undefined };
  if (prUnenriched) return { text: t("session.loadingPr"), tone: undefined };
  ...
  if (
    ...
    return {
      text:
        failed > 0
          ? t("session.checksFailed", { count: failed, plural: failed === 1 ? "" : "s" })
          : t("session.ciFailed"),
      tone: "fail",
    };
  }
  if (pr.reviewDecision === "changes_requested") {
    return { text: t("session.changesRequested"), tone: "amber" };
  }
  if (pr.unresolvedThreads > 0) {
    return {
      text: t("session.commentCount", {
        count: pr.unresolvedThreads,
        plural: pr.unresolvedThreads === 1 ? "" : "s",
      }),
      tone: "amber",
    };
  }
  if (isReadyToMerge && pr.reviewDecision === "approved") {
    return { text: t("session.approved"), tone: "green" };
  }
  if (pr.ciStatus === CI_STATUS.PASSING) return { text: t("session.ciPassed"), tone: "green" };
  if (pr.ciStatus === CI_STATUS.PENDING) return { text: t("session.ciPending"), tone: undefined };
  return { text: t("session.reviewPending"), tone: undefined };
}
