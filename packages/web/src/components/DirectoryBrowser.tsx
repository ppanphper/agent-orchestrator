    ...
    <>
      <div ref={toolbarRef} className="add-project-modal__toolbar">
        <div className="add-project-modal__toolbarcluster">
          ...
          <button type="button" onClick={browser.refresh} className="add-project-modal__toolbtn" aria-label={t("addProject.refresh")}>
            <RefreshIcon />
          </button>
          {browser.roots.length > 0 ? (
            <select
              aria-label={t("addProject.location")}
            ...
            >
              <option value="~">{t("addProject.home")}</option>
        ...
        <input
          aria-label={t("addProject.folderPath")}
      ...
      <div className="add-project-modal__content">
        <div ref={contentRef} className="add-project-browser">
          <div className="add-project-browser__current">
            <div className="add-project-browser__current-label">{t("addProject.currentFolder")}</div>
          ...
          {browser.error ? (
            <div className="add-project-browser__state add-project-browser__state--error">
              <p className="add-project-browser__state-title">{t("addProject.browserUnavailable")}</p>
            ...
            <div className="add-project-browser__rows">
              {showCurrentFolder ? (
                <button
                  ...
                  aria-label={
                    browser.currentDirectory?.isGitRepo
                      ? t("addProject.currentFolderGitAria", { name: currentName })
                      : t("addProject.currentFolderAria", { name: currentName })
                  }
                ...
                >
                  ...
                  <span className="add-project-browser__row-tag">{t("addProject.thisFolder")}</span>
                  {browser.currentDirectory?.isGitRepo ? (
              ...
              {browser.directoryEntries.length === 0 ? (
                <p className="add-project-browser__empty-row">{t("addProject.noFolders")}</p>
              ) : (
                browser.directoryEntries.map((entry) => {
                  ...
                  return (
                    <button
                      key={nextPath}
                      type="button"
                      aria-label={entry.isGitRepo ? t("addProject.entryGitAria", { name: entry.name }) : entry.name}
